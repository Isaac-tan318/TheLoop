...
router.get('/similar/:eventId', async (req, res, next) => {
  try {
    ...
    const targetEvent = await Event.findById(eventId).select('+embedding').lean({ virtuals: true });
    if (!targetEvent) {
      return res.status(404).json({ message: 'Event not found' });
    }

    // Get other upcoming events with embeddings
    ...
    // Get or generate target event embedding
    let targetEmbedding = targetEvent.embedding;
    if (!targetEmbedding?.length) {
      const targetText = buildEventText(targetEvent);
      targetEmbedding = await getEmbedding(targetText);
      // Cache for future use
      Event.updateOne({ _id: eventId }, { embedding: targetEmbedding }).catch(() => {});
    }

    // Score candidates using embeddings
    const scored = [];
    const eventsNeedingEmbedding = [];

    for (const event of candidateEvents) {
      if (event.embedding?.length) {
        scored.push({
          ...event,
          embedding: undefined,
          similarityScore: cosineSimilarity(targetEmbedding, event.embedding),
        });
      } else {
        eventsNeedingEmbedding.push(event);
      }
    }

    // Generate embeddings for events that don't have them
    if (eventsNeedingEmbedding.length > 0) {
      const texts = eventsNeedingEmbedding.map(buildEventText);
      const embeddings = await Promise.all(texts.map((text) => getEmbedding(text)));

      for (let i = 0; i < eventsNeedingEmbedding.length; i++) {
        const event = eventsNeedingEmbedding[i];
        const embedding = embeddings[i];
        // Cache embedding
        Event.updateOne({ _id: event._id }, { embedding }).catch(() => {});
        scored.push({
          ...event,
          embedding: undefined,
          similarityScore: cosineSimilarity(targetEmbedding, embedding),
        });
      }
    }
